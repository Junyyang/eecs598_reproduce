...
class Server:
    ...
    def init_paras(self):
        ...
        if self.args.model_type == 'NN':
            ...
            self.server_model = CNNCifar(num_classes = 2).to(self.args.device)
    ...
    def update_paras(self):
        self.get_grads()
        g_avg = self.average_grads() # average update
        for k in g_avg.keys():
            ...
            # args.model_type = CNN: standard CNN NN: multilayer perceptron 
                ...
                if acc_test >= self.args.target or i==(round-1):
                    open('data/results/accs_' + self.args.datatype + self.args.model_type + self.args.datatype + '_lr_' + str(self.args.sample_rate)
                        + 'target_acc_' + str(self.args.target) + '_sketch_type_' + self.args.sketchtype
                        +'_p_'+str(self.args.p)+'__'+self.args.runner, 'w').writelines(accs)
